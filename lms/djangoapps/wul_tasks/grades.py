...
class GradeReportBase(object):
    """
    Base class for grade reports (ProblemGradeReport and WulCourseGradeReport).
    """
    ...
    def send_mail_with_csv(self, context, success_rows, error_rows):
        """
        Creates and uploads a CSV for the given headers and rows.
        """
        date = datetime.now(UTC)
        send_attached_csv_by_mail(success_rows, context.file_name, context.course_id, date)
        if len(error_rows) > 1:
            send_attached_csv_by_mail(error_rows, context.file_name + '_err', context.course_id, date)
...
class _WulCourseGradeReportContext(object):
    """
...
class WulCourseGradeReport(object):
    """
...
class ProblemResponses(object):
    ...
    def generate(cls, _xmodule_instance_args, _entry_id, course_id, task_input, action_name):
        ...
        report_name = send_attached_csv_by_mail(rows, csv_name, course_id, start_date)
        current_step = {
            'step': 'CSV uploaded',
