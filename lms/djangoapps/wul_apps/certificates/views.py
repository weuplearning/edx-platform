...
def generate_pdf(request,course_id):
    ...
    try:
        user_name = (request.user.first_name).capitalize() + " " + (request.user.last_name).capitalize()
    except:
        try:
            user_name = json.loads(request.user.profile.custom_field).get('first_name') + " " + json.loads(request.user.profile.custom_field).get('last_name')
        except:
