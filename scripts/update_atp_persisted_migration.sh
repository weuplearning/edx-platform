...
echo "Run migrations for persisted grades"
sudo -E -H -u edxapp env "PATH=$PATH" /edx/bin/python.edxapp ./manage.py lms --settings=production makemigrations persisted_grades
sudo -E -H -u edxapp env "PATH=$PATH" /edx/bin/python.edxapp ./manage.py lms --settings=production migrate persisted_grades
